...
We bundled all required functions together in this version of the code. To recreate the plots in *Finite-size analysis of prepare-and-measure and decoy-state QKD via entropy accumulation* for realistic acceptance sets one would need to run the files `Run_qubit.m` for the qubit and `Run_decoy.m` for the decoy-state results. 
